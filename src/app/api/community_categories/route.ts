...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = new URL(`${API_BASE_URL}/api/community_categories`);
    ...
    if (!response.ok) {
      const maybeMessage =
        typeof data === "object" && data !== null && "message" in data
          ...
          : undefined;
      return NextResponse.json(
        { message: maybeMessage || "Failed to fetch community categories" },
    ...
    console.error("community_categories proxy error:", error);
    return NextResponse.json(
